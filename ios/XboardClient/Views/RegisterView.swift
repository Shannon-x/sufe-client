    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 16) {
                ...
                HStack {
                    ...
                    Button(action: sendCode) {
                        if sendingCode {
                            ...
                            Text(String(localized: "auth.email_code.send"))
                        }
                    }
                ...
                TextField(String(localized: "auth.invite_optional"), text: $inviteCode)
                    .textFieldStyle(.roundedBorder)
                    .textInputAutocapitalization(.never)
                    .autocorrectionDisabled()
                ...
                Button {
                    Task {
                        await model.register(
                            ...
                            password: password,
                            code: emailCode,
                            invite: inviteCode.isEmpty ? nil : inviteCode
                        )
                    }
                } label: {
                    if model.isAuthBusy {
                        ...
                        Text(String(localized: "auth.register.submit"))
